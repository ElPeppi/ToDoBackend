...
router.post("/login", async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ accessToken, refreshToken, user: { id: user.id, name: user.name, email: user.email }, tasks: tasksUser?.tasks || 0, completedTasks: tasksUser?.completedTasks || 0 });
  } catch (err) {
    console.error(err);
